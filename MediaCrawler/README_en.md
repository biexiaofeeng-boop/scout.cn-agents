...
```shell
# Recommended: set API key before starting WebUI API
export MEDIACRAWLER_API_KEY='change-this-to-a-strong-key'
...
> Security note: API auth is enabled by default (`MEDIACRAWLER_API_AUTH_ENABLED=true`) and docs are disabled by default. Set `MEDIACRAWLER_ENABLE_DOCS=true` if you need `/docs`.
