...
class RedisCache(AbstractCache):
    ...
    def get(self, key: str) -> Any:
        ...
        try:
            ...
            if isinstance(payload, dict) and "_mc_v" in payload:
                return payload.get("_mc_v")
            return payload
        except json.JSONDecodeError as exc:
            if os.getenv("MEDIACRAWLER_ALLOW_LEGACY_PICKLE", "").strip().lower() in {"1", "true", "yes"}:
                import pickle

                return pickle.loads(self._redis_client.get(key))
            raise ValueError(
                "Invalid cached payload format. Legacy pickle fallback is disabled."
            ) from exc
    ...
    def set(self, key: str, value: Any, expire_time: int) -> None:
        ...
        :return:
        """
        payload = json.dumps({"_mc_v": value}, ensure_ascii=False)
        self._redis_client.set(key, payload, ex=expire_time)
