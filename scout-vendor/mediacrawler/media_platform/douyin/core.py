...
class DouYinCrawler(AbstractCrawler):
    ...
    def _persistent_user_data_dir(self) -> str:
        return os.path.join(os.getcwd(), "browser_data", config.USER_DATA_DIR % config.PLATFORM)  # type: ignore

    def _backup_stale_user_data_dir(self) -> Optional[str]:
        user_data_dir = self._persistent_user_data_dir()
        if not os.path.exists(user_data_dir):
            return None
        backup_dir = f"{user_data_dir}.stale-{int(time.time())}"
        shutil.move(user_data_dir, backup_dir)
        utils.logger.warning(
            "[DouYinCrawler] Backed up stale persistent browser state from %s to %s",
            user_data_dir,
            backup_dir,
        )
        return backup_dir

    async def _assert_homepage_healthy(self) -> None:
        """Fail fast when Douyin returns an error/interstitial page caused by the local network environment."""
        ...
        try:
            ...
            pass
        try:
            body_text = (await self.context_page.locator("body").inner_text()).strip()
        except Exception:
            pass
        try:
        ...
        if "验证码中间页" in title or "验证码中间页" in body_text:
            raise RuntimeError(
                "[DouYinCrawler] Douyin persistent browser state landed on verification interstitial. "
                f"url={current_url} title={title!r} body_preview={body_preview!r}"
            )

        # Chromium error pages expose strong signals in the URL/title/body text. Avoid scanning
        # the full HTML for generic substrings like `err_`, because Douyin bundles include those
        # strings in normal JS assets and would otherwise trigger false positives.
        error_markers = (
        ...
        is_chromium_error_page = current_url.startswith("chrome-error://") or any(
            marker in title_lower or marker in body_lower for marker in error_markers
        )
        if is_chromium_error_page:
            raise RuntimeError(
                "[DouYinCrawler] Douyin returned a Chromium network error page. "
                f"url={current_url} title={title!r} body_preview={body_preview!r}. "
                "Check VPN/proxy/DNS/TLS environment first."
            )

        if not body_text and not title:
            raise RuntimeError(
                "[DouYinCrawler] Douyin page is blank after navigation. "
                f"url={current_url}. This usually indicates a local network/proxy interception issue."
            )

        if len(content) < 2000 and len(body_text) < 20:
            raise RuntimeError(
                "[DouYinCrawler] Douyin page payload is unexpectedly small. "
                f"url={current_url} title={title!r} body_preview={body_preview!r}. "
                "Check whether the current network exit is being blocked or challenged."
            )
    ...
    async def start(self) -> None:
        ...
        async with async_playwright() as playwright:
            ...
            try:
                await self._assert_homepage_healthy()
            except RuntimeError as exc:
                can_retry_with_fresh_profile = (
                    not config.ENABLE_CDP_MODE
                    and config.SAVE_LOGIN_STATE
                    and "persistent browser state landed on verification interstitial" in str(exc)
                )
                if not can_retry_with_fresh_profile:
                    raise

                utils.logger.warning(
                    "[DouYinCrawler] Persistent profile appears stale, recreating a clean dy_user_data_dir and retrying once ..."
                )
                await self.browser_context.close()
                self._backup_stale_user_data_dir()
                self.browser_context = await self.launch_browser(
                    chromium,
                    playwright_proxy_format,
                    user_agent=None,
    ...
    async def create_douyin_client(self, httpx_proxy: Optional[str]) -> DouYinClient:
        ...
        try:
            ...
            utils.logger.warning(
                "[DouYinCrawler.create_douyin_client] Failed to read navigator.userAgent from page context, fallback to generated UA: %s",
                exc,
            )
