    ...
    async def login_by_cookies(self):
        ...
        if not cookie_dict:
            utils.logger.warning(
                "[XiaoHongShuLogin.login_by_cookies] cookie string is empty; this mode only works if persistent browser state is already logged in."
            )
            return
        ...
        if injected == 0:
            utils.logger.warning(
                "[XiaoHongShuLogin.login_by_cookies] no supported xhs auth cookie was injected; browser state must already contain a valid login."
            )
