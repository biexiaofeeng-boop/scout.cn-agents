        ...
        async with async_playwright() as playwright:
            ...
            try:
                ...
                utils.logger.warning(
                    "[XiaoHongShuCrawler.start] Homepage load timed out on full readiness; continuing because DOM may already be usable."
                )
            ...
            if not await self.xhs_client.pong():
                ...
                if not await self.xhs_client.pong():
                    raise DataFetchError(
                        "XHS login state is still invalid after login attempt. "
                        "Cookie mode requires a valid browser state or an interactive qrcode login."
                    )
