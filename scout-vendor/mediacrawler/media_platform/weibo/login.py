...
class WeiboLogin(AbstractLogin):
    QR_ENTRY_SELECTORS = (
        "text=扫码登录",
        "text=扫描二维码登录",
        "xpath=//span[contains(., '扫码登录')]",
        "xpath=//a[contains(., '扫码登录')]",
    )

    QRCODE_SELECTORS = (
        "xpath=//img[@class='w-full h-full']",
        "css=img.w-full.h-full",
        "xpath=//div[contains(., '扫描二维码登录')]/following::img[contains(@src, 'qr.weibo.cn')][1]",
        "xpath=//img[contains(@src, 'qr.weibo.cn')]",
    )
    ...
    async def login_by_qrcode(self):
        ...
        # Mobile UA lands on SMS login first. Switch to QR login surface when needed.
        if "扫描二维码登录" not in await self.context_page.locator("body").inner_text():
            for selector in self.QR_ENTRY_SELECTORS:
                try:
        ...
        # find login qrcode
        base64_qrcode_img = ""
        for qrcode_img_selector in self.QRCODE_SELECTORS:
