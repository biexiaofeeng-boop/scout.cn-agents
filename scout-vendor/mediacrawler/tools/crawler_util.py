...
async def find_login_qrcode(page: Page, selector: str) -> str:
    ...
    try:
        ...
        login_qrcode_img = ""
        try:
            src_handle = await elements.get_property("src")  # type: ignore
            login_qrcode_img = str(await src_handle.json_value() or "")
        except Exception:
            login_qrcode_img = ""

        if not login_qrcode_img:
            try:
                login_qrcode_img = str(await elements.evaluate("(el) => el.currentSrc || el.src || ''"))  # type: ignore
            except Exception:
                login_qrcode_img = ""
