...
1. `config.yaml` 中不要写死数据库密码，改用环境变量覆盖（`WECHAT_MYSQL_PASSWD`、`WECHAT_REDIS_PASSWD`）
2. `service_host` 建议绑定 `127.0.0.1` 或内网地址，不要直接暴露公网
3. 如使用 `docker-compose`，请在启动前设置 `.env` 中的数据库口令，不要使用默认值
