...
class TaskManager():
    ...
    def __get_task_from_redis(self, key):
        ...
        if task:
            ...
            try:
                ...
                # backward compatibility for historic cache values like \"{'k': 'v'}\"
                try:
                    parsed = ast.literal_eval(raw_task)
                    if isinstance(parsed, dict):
                        return parsed
                    return None
                except Exception:
                    log.error('redis task parse failed: %s', raw_task)
                    return None
    ...
    def get_account_task(self):
        ...
        if not task:
            publish_time_condition = "AND last_publish_time < %s" if self._ignore_haved_crawl_today_article_account else ''
            sql = '''
                SELECT
                    __biz,
                    last_publish_time
                ...
                '''.format(monitor_interval='%s', publish_time_condition=publish_time_condition)

            params = [1, self._monitor_interval]
            if self._ignore_haved_crawl_today_article_account:
                params.append(tools.get_current_date(date_format='%Y-%m-%d' + ' 00:00:00'))

            tasks = self._mysqldb.find(sql, params=params, to_json=True)
            if tasks:
    ...
    def update_article_task_state(self, sn, state=1):
        sql = 'update wechat_article_task set state = %s where sn = %s'
        self._mysqldb.update(sql, (state, sn))
    ...
    def is_reach_last_article_publish_time(self, __biz, publish_time):
        ...
        if not last_publish_time:
            # 查询mysql里是否有该任务
            sql = "select last_publish_time from wechat_account_task where __biz = %s"
            data = self._mysqldb.find(sql, (__biz,))
            if data:  # [(None,)] / []
    ...
    def update_account_last_publish_time(self, __biz, last_publish_time):
        sql = 'update wechat_account_task set last_publish_time = %s, last_spider_time=%s where __biz=%s'
        self._mysqldb.update(sql, (last_publish_time, tools.get_current_date(), __biz))
    ...
    def sign_account_is_zombie(self, __biz, last_publish_time=None):
        if last_publish_time:
            sql = 'update wechat_account_task set last_publish_time = %s, last_spider_time=%s, is_zombie=1 where __biz=%s'
            params = (last_publish_time, tools.get_current_date(), __biz)
        else:
            sql = 'update wechat_account_task set last_spider_time=%s, is_zombie=1 where __biz=%s'
            params = (tools.get_current_date(), __biz)

        self._mysqldb.update(sql, params)
    ...
    def get_task(self, url=None, tip=''):
        ...
        if url:
            ...
            next_page = "{tip} 休眠 {sleep_time}s 下次刷新时间 {begin_spider_time} <script>setTimeout(function(){{window.location.href='{url}';}},{sleep_time_msec});</script>".format(
                tip=tip and tip + ' ', sleep_time=sleep_time, begin_spider_time=tools.timestamp_to_date(tools.get_current_timestamp() + sleep_time), url=safe_url, sleep_time_msec=sleep_time * 1000
            )
        else:
